...
vim.api.nvim_buf_set_lines(buf, 0, -1, false, { string.rep("a", 150) })
...
vim.keymap.set("n", "<cr>", function()
    ...
    local row = view.topline - 1
    local vcol = view.skipcol
    vim.notify(tostring(vim.api.nvim_win_text_height(0, { end_row = row, end_vcol = vcol }).all))
end)
-- print(vim.api.nvim_win_get_width(winnr))
